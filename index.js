...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan qr kode sekarang !!!'))
	})
	client.on('credentials-updated', () => {
		fs.writeFileSync('./BarBar.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'Memeriksa update tan')
	})
	fs.existsSync('./BarBar.json') && client.loadAuthInfo('./BarBar.json')
	client.on('connecting', () => {
		start('2', 'Tunggu sabar ya sebentar!!!')
	})
	client.on('open', () => {
		success('2', 'Sudah terconned')
	})
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const adminbotnumber = ["6282137409151@s.whatsapp.net"]
			const frendsowner = ["6282137409151@s.whatsapp.net"]
			...
			case 'owner':
                 ...
                 client.sendMessage(from, 'Nih owner gue jangan lupa SV ya',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'report':
                    ...
                    client.sendMessage('6282137409151@s.whatsapp.net', options, text, {quoted: mek})
				...
				case 'loli':
				    try {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih lolinya om...'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
			    ...
			    case 'waifu':
				    try {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Waifu nya nih.....'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
			    ...
			    case 'randomanime':
				    try {
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih Anime nya.....'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
               ...
               case 'infogempa':
				...
				case 'kurumi':
					...
					break
				case 'miku':
					if (!isAnime) return reply(' *Anda harus Mengaktifkan Mode Anime* ❌')
				...
				case 'hentai':
					...
					client.sendMessage(from, nye, image, { caption: 'Tobat Ya Allah...', quoted: mek })
					break
				case 'loli2':
					if (!isAnime) return reply(' *Anda belum Mengaktifkan Mode Anime* ❌')
					...
					client.sendMessage(from, nye, image, { caption: 'Nih lolinya om....', quoted: mek })
					break
				case 'anjing':
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘 GUSTIAN BOT 〙', members_id, true)
					break
				case 'clearall':
					if (!isOwner) return reply('Kamu siapa?')
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Gunakan kode negara ya !!!')
					try {
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih Toimg nya.....'})
						fs.unlinkSync(ran)
					})
					break
				case 'simi':
					if (args.length < 1) return reply('Textnya mana  woi pake text jh?')
					teks = body.slice(5)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('ketik : 1 untuk mengaktifkan, ketik : 0 untuk menonaktifkan')
					}
					break
			    case 'nsfw':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'openanime':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Harus 1 = on 0 =off')
					if (Number(args[0]) === 1) {
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Udah aktif woi jangan ulang lagi.')
						welkom.push(from)
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					prefix = args[0]
					reply(`Hore 👏Prefix berhasil di ubah menjadi : ${prefix}`)
					break
		        //fitur adminbot
		        case 'setpp2':
                    if (!isGroup) return reply(mess.only.group)
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner AHMAD!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin AHMAD!')
				   }
                   break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Fotonya mana harus pake foto!!')
					}
					break
				default:
